...
public final class NVActivityIndicatorView: UIView {
    ...
    ///
    /// - note:
    /// Default time that has to be elapsed (between calls of `startAnimating()` and `stopAnimating()`)
    ///  in order to actually display UI blocker. It should be set thinking about what the
    ///  minimum duration of an activity is to be worth showing it to the user. If the activity
    ///  ends before this time threshold, then it will not be displayed at all.
    public static var DEFAULT_BLOCKER_DISPLAY_TIME_THRESHOLD = 0

    /// Default minimum display time of UI blocker. Default value is 0 ms.
    ///
    /// - note:
    /// Default minimum display time of UI blocker. Its main purpose is to avoid flashes showing
    /// and hiding it so fast. For instance, setting it to 200ms will force UI blocker to be shown
    /// for at least this time (regardless of calling `stopAnimating()` ealier).
    ...
    private final func setUpAnimation() {
        ...
        #else
        var animationRect = UIEdgeInsetsInsetRect(
            frame,
            UIEdgeInsets(
                top: padding,
                left: padding,
