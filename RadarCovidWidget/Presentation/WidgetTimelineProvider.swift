...
struct WidgetTimelineProvider: TimelineProvider {
    ...
    func placeholder(in context: Context) -> WidgetTimelineEntry {
        WidgetTimelineEntry(exposition: presenter.expositionInfo, date: Date())
    }

    func getSnapshot(in context: Context, completion: @escaping (WidgetTimelineEntry) -> ()) {
        let entry = WidgetTimelineEntry(exposition: presenter.expositionInfo, date: Date())
        completion(entry)
    }
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<WidgetTimelineEntry>) -> ()) {
        ...
        for hourOffset in 0..<5 {
            ...
            let entry = WidgetTimelineEntry(exposition: presenter.expositionInfo, date: entryDate)
